...
const PriceRate = styled.span<{ isMinus: boolean }>`
  color: ${props => props.isMinus ? props.theme.accentColor : props.theme.subColor};
  font-size: 18px;
  font-weight: bold;
`;

function Price({ coinId }: coinIdProps) {
  const { isLoading, data } = useQuery<IPrice>(["todayPrice", coinId], () => fecthCoinPrice(coinId));
  return (
    <PriceContainer>
      {isLoading ? "Loading Price..." :
        <>
          ...
          <PriceItem>
            ...
            <PriceRate
              isMinus={data?.quotes.USD.percent_change_12h.toString().slice(0, 1) !== "-"}
            >
          ...
          <PriceItem>
            ...
            <PriceRate
              isMinus={data?.quotes.USD.percent_change_24h.toString().slice(0, 1) !== "-"}
            >
          ...
          <PriceItem>
            ...
            <PriceRate
              isMinus={data?.quotes.USD.percent_change_7d.toString().slice(0, 1) !== "-"}
            >
          ...
          <PriceItem>
            ...
            <PriceRate
              isMinus={data?.quotes.USD.volume_24h_change_24h.toString().slice(0, 1) !== "-"}
            >
